...
PTR randomseed;                     /* Current random number seed. */
...
/*  triangles are four-byte-aligned and that the `PTR' type is     */
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((PTR) (ptr) & (PTR) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((PTR) (ptr) ^ (PTR) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((PTR) (otri).tri | (PTR) (otri).orient)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((PTR) (sptr) & (PTR) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((PTR) (sptr) & ~ (PTR) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((PTR) (osub).ss | (PTR) (osub).ssorient)
...

{
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (PTR) printvertex,
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (PTR) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
...

{
  ...
  alignptr = (PTR) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (PTR) pool->alignbytes -
     (alignptr % (PTR) pool->alignbytes));
...

{
  ...
  alignptr = (PTR) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (PTR) m->triangles.alignbytes -
     (alignptr % (PTR) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (PTR) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (PTR) m->subsegs.alignbytes -
       (alignptr % (PTR) m->subsegs.alignbytes));
...

{
  ...
  alignptr = (PTR) (getblock + 1);
  foundvertex = (char *) (alignptr + (PTR) m->vertices.alignbytes -
                          (alignptr % (PTR) m->vertices.alignbytes));
...
#ifdef ANSI_DECLARATORS
PTR randomnation(unsigned int choices)
#else /* not ANSI_DECLARATORS */
PTR randomnation(choices)
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (PTR) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (PTR) m->triangles.alignbytes -
                         (alignptr %
                          (PTR) m->triangles.alignbytes));
