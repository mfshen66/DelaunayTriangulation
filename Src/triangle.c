...

{
  ...
  unsigned long long alignptr = (unsigned long long)(*(pool->nowblock) + 1);
...

{
  ...
  VOID* mempool = trimalloc(pool->itemsfirstblock * pool->itembytes + (int)sizeof(VOID*) +
      pool->alignbytes);
  pool->firstblock = &mempool;
